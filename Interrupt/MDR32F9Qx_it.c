...
void UART2_IRQHandler(void)
{
	  ...
	  if (UART_GetITStatusMasked(MDR_UART2, UART_IMSC_RXIM) == SET){
    UART_ClearITPendingBit(MDR_UART2, UART_IT_RX);

//		rcvData = UART_ReceiveData(MDR_UART2);
//    FIFO_PUSH(RCVbuff,tmp);
  }
		
	//прерывание на передачу
//	if (UART_GetITStatusMasked(MDR_UART1, UART_IT_TX) == SET){
//		UART_ClearITPendingBit(MDR_UART1, UART_IT_TX);
//		if(cntDataToSend--)sendByteInterrupt((uint8_t)*(dataToSendInterrupt));
//	
//	}
}
/*******************************************************************************
...
//static uint8_t cntVoltageError = 0;
//	float V_PWR = 0;
////	int temperature_C;

//	//Если измерено напряжение питания
//	if(ADC1_GetFlagStatus(ADCx_IT_END_OF_CONVERSION) == SET){
//		V_Value	= MDR_ADC->ADC1_RESULT & 0x00000FFF;
//		//V_Value *=11;
//		//V_Value /=10;
//		V_PWR = V_Value*3.3*11/4095;
//		statusMCU.PWR = (uint8_t)V_PWR;
//		if(IS_MCU_EN_RFPA )
//			if( (statusCAN.statusControlIN & (1<<STATUSBIT_MCU_EN_RFPA))){//Отрабатываем ошибку только если включен передатчик
//				if((V_Value < ADC_MIN) || (V_Value > ADC_MAX)){
//					if(cntVoltageError++ > 8)
//								flagAvariaPWR = 1;
//					/*POWER_BAD;*/
//				}
//				else {
//					cntVoltageError = 0;//сбросили счетчик ошибок
//					flagAvariaPWR = 0;
//						/*POWER_GOOD;*/
//				}
//			}
//	}
//if(MDR_ADC->ADC1_STATUS & ADC_STATUS_FLG_REG_OVERWRITE)	MDR_ADC->ADC1_STATUS |= ADC_STATUS_FLG_REG_OVERWRITE;
